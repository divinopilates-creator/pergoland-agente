...
class ProveedorWhapi(ProveedorWhatsApp):
    ...
    async def parsear_webhook(self, request: Request) -> list[MensajeEntrante]:
        ...
        for msg in body.get("messages", []):
            ...
            # Mensajes de texto — flujo normal (incluye los de Gabriel, es_propio=True)
            if tipo == "text":
            ...
            # Si lo manda el cliente (es_propio=False): se trata como respuesta de material, Matías sigue el flujo.
            # Si lo manda Gabriel (es_propio=True): se registra como intervención humana, no se descarta.
            elif tipo in TIPOS_ARCHIVO:
                if es_propio:
                    logger.info(f"Archivo enviado por Gabriel tipo={tipo} a {msg.get('chat_id', '')} — registrado como intervención humana")
                else:
